...
bool MouseOver(float x, float y, vec2 &p, int proximity, int xCursorOffset, int yCursorOffset) {
	return length(vec2(x+xCursorOffset, y+yCursorOffset)-p) < proximity;
}

bool MouseOver(float x, float y, vec3 &p, mat4 &view, int proximity, int xCursorOffset, int yCursorOffset) {
...
void Framer::Wheel(float direction, bool shift) { arcball.Wheel(direction, shift); }
...
void Joystick::Down(int x, int y, vec3 *b, vec3 *v, mat4 modelview, mat4 persp) {
	...
	mode = ScreenDistSq(x, y, *base, fullview) < 100? A_Base :
	       ScreenDistSq(x, y, *base+*vec, fullview) < 100? A_Tip : A_None;
	if (mode == A_Base)
